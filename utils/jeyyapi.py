...
class JeyyAPIClient:
	...
	def glitch(self, image_url: str, level: typing.Optional[int] = 3) -> BytesIO:
		return self._image_fetch('glitch', image_url=image_url, level=int(level))
	
	def boil(self, image_url: str, level: typing.Optional[str] = 2) -> BytesIO:
		return self._image_fetch('boil', image_url=image_url, level=int(level))
	
	def earthquake(self, image_url: str, level: typing.Optional[int] = 3) -> BytesIO:
		return self._image_fetch('earthquake', image_url=image_url, level=int(level))

	def hearts(self, image_url: str, rainbow: typing.Optional[bool] = True) -> BytesIO:
		return self._image_fetch(
			'hearts', image_url=image_url, rainbow=str(bool(rainbow))
		)
	...
	def shear(self, image_url: str, axis: typing.Optional[str] = 'X') -> BytesIO:
		return self._image_fetch('shear', image_url=image_url, axis=str(axis))
	...
	def print(self, image_url: str) -> BytesIO:
		return self._image_fetch('print', image_url=image_url)

	def matrix(self, image_url: str) -> BytesIO:
		return self._image_fetch('matrix', image_url=image_url)

	def sensitive(self, image_url: str) -> BytesIO:
		return self._image_fetch('sensitive', image_url=image_url)
	
	def dilute(self, image_url: str) -> BytesIO:
		return self._image_fetch('dilute', image_url=image_url)

	def pattern(self, image_url: str) -> BytesIO:
		return self._image_fetch('pattern', image_url=image_url)

	def logoff(self, image_url: str) -> BytesIO:
		return self._image_fetch('logoff', image_url=image_url)

	def ace(self, name: str, side: typing.Literal['attorney', 'prosecutor'], text: str) -> BytesIO:
		return self._image_fetch('ace', name=name, side=str(side), text=text)
	...
	def paparazzi(self, image_url: str) -> BytesIO:
		return self._image_fetch('paparazzi', image_url=image_url)
	...
	def clock(self, image_url: str) -> BytesIO:
		return self._image_fetch('clock', image_url=image_url)
	...
	def warp(self, image_url: str) -> BytesIO:
		return self._image_fetch('warp', image_url=image_url)

	def ads(self, image_url: str) -> BytesIO:
		return self._image_fetch('ads', image_url=image_url)

	def bubble(self, image_url: str) -> BytesIO:
		return self._image_fetch('bubble', image_url=image_url)

	def cloth(self, image_url: str) -> BytesIO:
		return self._image_fetch('cloth', image_url=image_url)

	def youtube(self, avatar_url: str, author: str, title: str) -> BytesIO:
		return self._image_fetch(
			'youtube', avatar_url=avatar_url, author=author, title=title
		)
